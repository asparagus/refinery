...
from refinery import Feedback, FeedbackModule, Input, Output, Predictor, Retrier, Validation
...
class QAModule(Predictor[QA]):
    ...
    def __init__(self, validation: Validation[QA] | None = None, callbacks: Any | None = None):
...
class QAFeedback(FeedbackModule[QA]):
    ...
    def forward(self, input: Question, output: Answer, _trace = None) -> Feedback:
        if count := output.answer.count(" "):
            return Feedback(evaluation=1. / (1. + count), feedback="The answer is too long")
...
if __name__ == "__main__":
    ...
    if args.validation:
        module.validation = Validation(feedback_module=feedback_module, validation_threshold=1.0)
    ...
    evaluation = dspy.Evaluate(devset=trainset, metric=feedback_module, provide_traceback=True)
